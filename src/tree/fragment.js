...
import { iterator, prayWellFormed } from '../constants.js';
...
export class Fragment {
    ...
    ends = {};
    disowned;
    each = iterator((yield_) => {
        let el = this.ends.left;
        if (!el)
            return this;
        for (; el !== this.ends.right?.right; el = el.right) {
            if (!el)
                throw new Error('each unable to continue right in Fragment');
            if (yield_(el) === false)
                break;
        }
        return this;
    ...
    constructor(withDir, oppDir, dir = 'left') {
        if (!withDir !== !oppDir)
            throw new Error('no half-empty fragments');
        if (!withDir)
            return;
        if (!(withDir instanceof TNode))
            throw new Error('withDir must be passed to Fragment');
        if (!(oppDir instanceof TNode))
            throw new Error('oppDir must be passed to Fragment');
        if (withDir.parent !== oppDir.parent)
            throw new Error('withDir and oppDir must have the same parent');
        if (dir === 'left') {
    ...
    adopt(parent, leftward, rightward) {
        ...
        if (this.ends.right)
            this.ends.right.right = rightward;
        this.each((el) => {
            el.left = leftward;
            el.parent = parent;
            if (leftward)
    ...
    disown() {
        ...
        if (!leftEnd || this.disowned)
            return this;
        this.disowned = true;
        const rightEnd = this.ends.right;
        const parent = leftEnd.parent;
        if (!parent)
            throw new Error('a parent must always present');
