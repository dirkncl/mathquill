...
import { noop, mqCmdId, mqBlockId, LatexCmds, OPP_BRACKS, BRACKET_NAMES, SVG_SYMBOLS, BuiltInOpNames, TwoWordOpNames, otherDir } from '../constants.js';
import { Parser } from '../services/parser.util.js';
import { Selection } from '../selection.js';
import { deleteSelectTowardsMixin, DelimsMixin } from '../mixins.js';
import { Point } from '../tree/point.js';
...
export class MathCommand extends deleteSelectTowardsMixin(MathElement) {
    ...
    placeCursor(cursor) {
        ...
        cursor.insAtRightEnd(this.foldChildren(this.ends.left, (leftward, child) => (leftward.isEmpty() ? leftward : child)));
    }
    selectChildren() {
        return new Selection(this, this);
    }
    unselectInto(dir, cursor) {
        cursor.insAtDirEnd(otherDir(dir), cursor.anticursor?.ancestors?.[this.id]);
    }
    seek(pageX, cursor) {
    ...
    html() {
        ...
        if (tokens.join('') !== this.htmlTemplate)
            throw new Error('no unmatched angle brackets');
        ...
        for (let i = 0, token = tokens[0]; token; ++i, token = tokens[i]) {
            ...
            // top-level open tags
                ...
                if (token.charAt(1) === '/')
                    throw new Error('unmatched top-level close tag');
        ...
        return tokens
            .join('')
            .replace(/>&(\d+)/g, (_$0, $1) => ` ${mqBlockId}=${blocks[$1]?.id.toString() ?? ''}>${blocks[$1]?.join('html') ?? ''}`);
    }
    ...
    mathspeak() {
        let i = 0;
        return this.foldChildren(`${this.mathspeakTemplate[i] || `Start${this.ctrlSeq.replace(/^\\/, '')}`} `, (speech, block) => {
            ++i;
            return `${speech} ${block.mathspeak()} ${this.mathspeakTemplate[i] || `End${this.ctrlSeq.replace(/^\\/, '')}`} `;
        });
    }
}
...
export class Symbol extends MathCommand {
    ...
    moveTowards(dir, cursor) {
        if (dir === 'left')
            this.elements.first.before(cursor.element);
        else
            this.elements.last.after(cursor.element);
    ...
    seek(pageX, cursor) {
        ...
        if (pageX - rect.left < rect.width / 2)
            cursor.insLeftOf(this);
        else
            cursor.insRightOf(this);
    }
...
export class VanillaSymbol extends Symbol {
    constructor(ch, html, text, mathspeak) {
        super(ch, `<span>${html || ch}</span>`, text, mathspeak);
    }
}
export class BinaryOperator extends Symbol {
    isUnary = false;
    constructor(ctrlSeq, html, text, mathspeak, useRawHtml = false) {
...
export class Inequality extends BinaryOperator {
    ...
    constructor(data, strict) {
        ...
        super(data[`ctrlSeq${strictness}`], data[`html${strictness}`], data[`text${strictness}`], data[`mathspeak${strictness}`]);
        this.data = data;
        this.strict = strict;
    }
    swap(strict) {
...
export class FactorialOrNEQ extends Inequality {
    ...
    contactWeld() {
        if (this.isUnary)
            this.elements.firstElement.className = '';
        else
            this.elements.firstElement.className = 'mq-binary-operator';
        return this;
    }
}
...
export class Letter extends Variable {
    letter;
    constructor(ch, htmlTemplate) {
        super(ch, htmlTemplate);
        this.letter = ch;
        this.siblingDeleted = this.siblingCreated = (opts, dir) => {
    ...
    finalizeTree(opts, dir) {
        ...
        if (dir !== 'left' && this.right instanceof Letter)
            return;
    ...
    autoUnItalicize(opts) {
        ...
        // sequence of letters
        let str = this.letter, l = this.left, r = this.right;
        for (; l instanceof Letter; l = l.left)
            str = l.letter + str;
        for (; r instanceof Letter; r = r.right)
            str += r.letter;
        // removeClass and delete flags from all letters before figuring out
        // which, if any, are part of an operator name
        new Fragment(l?.right || this.parent?.ends.left, r?.left || this.parent?.ends.right).each((el) => {
            el.italicize(true).elements.removeClass('mq-first', 'mq-last', 'mq-followed-by-supsub');
        ...
        for (let i = 0, first = l?.right || this.parent?.ends.left; i < str.length; ++i, first = first?.right) {
            for (let len = Math.min(autoOps._maxLength, str.length - i); len > 0; --len) {
                ...
                if (autoOps[word]) {
                    ...
                    if (word in TwoWordOpNames)
                        last?.left?.left?.left?.elements.addClass('mq-last');
                    if (!this.shouldOmitPadding(first?.left))
                        first?.elements.addClass('mq-first');
                    if (!this.shouldOmitPadding(last?.right)) {
                        const rightward = last?.right;
                        if (rightward instanceof SupSub)
                            rightward.siblingCreated?.(opts);
                        else
                            last?.elements.toggleClass('mq-last', !(rightward instanceof Bracket));
                    }
                    i += len - 1;
                    first = last;
                    break;
...
export const getCtrlSeqsFromBlock = (block) => {
    if (!block)
        return '';
    let chars = '';
...
export class Fraction extends MathCommand {
    ...
    text() {
        ...
        const text = (dir) => {
            ...
            let numPeriods = 0;
            this.ends[dir]?.eachChild((child) => {
                if (child instanceof Digit)
                    haveDigits = true;
                if (child instanceof VanillaSymbol && child.ctrlSeq === '.')
                    ++numPeriods;
                if (!(child instanceof Digit ||
    ...
    getFracDepth() {
        const level = 0;
        const walkUp = function (item, level) {
...
export const supSubText = (prefix, block) => {
    let needParens = false;
    let numBlocks = 0;
    let haveDigits = false;
    let numPeriods = 0;
...
export class SupSub extends MathCommand {
    supsub = 'sup';
    constructor(_ctrlSeq, htmlTemplate, textTemplate) {
        ...
        this.reflow = () => {
            ...
            if (sup) {
                ...
                const sup_bottom = supRect.top + supRect.height - parseFloat(supStyle.paddingTop) - parseFloat(supStyle.paddingBottom);
        ...
        this.siblingCreated = this.siblingDeleted = (options) => {
            if (this.left instanceof Letter &&
                ...
                this.elements.removeClass('mq-after-operator-name');
            this.maybeFlatten(options);
        };
    }
    hasValidBase(options, leftward, parent) {
    ...
    createLeftOf(cursor) {
        if (this.hasValidBase(cursor.options, cursor.left, cursor.parent)) {
            ...
            if (cursor.left instanceof Fraction) {
                ...
                cursor.selection = cursor.left.selectChildren();
        ...
        if (this.replacedFragment) {
            if (cursor.parent)
                this.replacedFragment.adopt(cursor.parent, cursor.left, cursor.right);
    ...
    contactWeld(cursor) {
            ...
            if (this[dir] instanceof SupSub) {
                ...
                for (const supsub of ['sub', 'sup']) {
                    ...
                    if (!dest)
                        ...
                        if (dir === 'left')
                            children.adopt(dest, dest.ends.right);
                        else
                            children.adopt(dest, undefined, dest.ends.left);
                    }
                    else
                        pt = new Point(dest, undefined, dest.ends.left);
        ...
        if (this.parent?.parent instanceof MathFunction &&
        ...
        if (!this.hasValidBase(options, leftward, this.parent) || leftward instanceof Fraction) {
            ...
            if (leftward === cursor?.left) {
                if (cursor?.left?.right)
                    cursor.insLeftOf(cursor.left.right);
                else if (cursor?.parent)
                    cursor.insAtDirEnd('left', cursor.parent);
            }
        }
    }
    ...
    deleteTowards(dir, cursor) {
        if (cursor.options.autoSubscriptNumerals && this.sub) {
            ...
            if (cmd instanceof Symbol)
                ...
                cmd.deleteTowards(dir, cursor.insAtDirEnd(otherDir(dir), this.sub));
            // TODO: factor out a .removeBlock() or something
            if (this.sub.isEmpty()) {
                this.sub.deleteOutOf('left', cursor.insAtLeftEnd(this.sub));
                if (this.sup)
                    cursor.insDirOf(otherDir(dir), this);
    ...
    addBlock(block) {
        ...
        for (const supsub of ['sub', 'sup']) {
            ...
            if (this.parent?.parent instanceof MathFunction && this.parent === this.parent.parent.blocks[0]) {
                ...
                thisSupsub.deleteOutOf = (dir, cursor) => {
                    if (thisSupsub.isEmpty()) {
                        ...
                        if (updown === 'up')
                            ...
                            delete this.downInto;
                        const remainingSupsub = this[oppositeSupsub];
                        remainingSupsub[`${updown}OutOf`] = insLeftOfMeUnlessAtEnd;
                        remainingSupsub.deleteOutOf = (dir, cursor) => {
                            if (remainingSupsub.isEmpty())
                                this.remove();
                            if (this.parent)
                    ...
                    if (thisSupsub.parent)
                        cursor.insDirOf(thisSupsub[dir] ? otherDir(dir) : dir, thisSupsub.parent);
                };
            }
            else {
                thisSupsub.deleteOutOf = (dir, cursor) => {
                    ...
                    if (updown === 'up')
                        ...
                        delete this.downInto;
                    if (this[oppositeSupsub])
                        this[oppositeSupsub][`${updown}OutOf`] = insLeftOfMeUnlessAtEnd;
                    delete this[oppositeSupsub].deleteOutOf;
                    if (supsub === 'sub') {
                        this.elements.addClass('mq-sup-only');
                        this.elements.first.lastChild?.remove();
                    }
...
export class UpperLowerLimitCommand extends MathCommand {
    ...
    mathspeak() {
        return `Start ${this.ariaLabel ?? this.ctrlSeq.replace(/^\\/, '')} from ${this.ends.left?.mathspeak() ?? ''} to ${this.ends.right?.mathspeak() ?? ''}, End ${this.ariaLabel ?? this.ctrlSeq.replace(/^\\/, '')}, `;
    }
    parser() {
        ...
        return Parser.optWhitespace
            ...
            return latexMathParser.block.then((block) => {
...
const BracketMixin = (Base) => class extends DelimsMixin(Base) {
    side = 'left';
    sides = { left: { ch: '(', ctrlSeq: '(' }, right: { ch: ')', ctrlSeq: ')' } };
    inserted = false;
    ...
    createLeftOf(cursor) {
        ...
        if (brack) {
            ...
            if (this instanceof MathFunction && side === 'left') {
                ...
                const rightward = brack === cursor.parent?.parent && cursor.right !== brack.ends.right?.right
                    ? new Fragment(cursor.right, brack.ends.right?.ends.right, 'left').disown()
                    : undefined;
                cursor.insRightOf(brack);
                delete this.side;
                if (rightward)
                    this.replaces(rightward);
                super.createLeftOf(cursor);
                brack.remove();
                this.bubble('reflow');
                return;
            ...
            if (this instanceof MathFunction &&
        ...
        if (side === 'left' && brack.ends.left)
            cursor.insAtLeftEnd(brack.ends.left);
        else
            cursor.insRightOf(brack);
    }
    unwrap() {
        const node = this.parent
            ...
            : undefined;
        if (node)
            this.elements.last.after(...node.elements.contents);
        this.remove();
    }
    deleteSide(side, outward, cursor) {
        ...
        if (this.matchBrack(cursor.options, side, this.blocks[this.contentIndex].ends[this.side])) {
            this.closeOpposing(this.blocks[this.contentIndex].ends[this.side]);
            ...
            if (sib) {
                ...
                if (outward)
                    ...
                    cursor.insAtDirEnd(side, this.blocks[this.contentIndex]);
            }
        }
    }
    replaceBracket(brackFrag, side) {
        ...
        if (side === 'left') {
            ...
            if (next instanceof HTMLElement)
                next.style.marginLeft = symbol.width;
        }
        else {
            const prev = brackFrag.previousElementSibling;
            if (prev instanceof HTMLElement)
                prev.style.marginRight = symbol.width;
        }
    }
    getSymbol(side) {
        return (SVG_SYMBOLS[this.sides[side || 'right'].ch] || {
            width: '0',
            html: ''
        });
    }
    ...
    finalizeTree() {
        if (!this.inserted) {
            this.blocks[this.contentIndex].deleteOutOf = (dir, cursor) => {
...
export class Bracket extends BracketMixin(MathCommand) {
    constructor(side, open, close, ctrlSeq, end) {
        ...
        this.siblingCreated = (_opts, dir) => {
            if (dir === otherDir(this.side))
                this.finalizeTree();
        };
    }
    numBlocks() {
    ...
    html() {
        ...
        this.htmlTemplate =
            '<span class="mq-bracket-container mq-non-leaf">' +
                `<span class="mq-bracket-l mq-scaled mq-paren${this.side === 'right' ? ' mq-ghost' : ''}" style="width:${leftSymbol.width}">` +
                leftSymbol.html +
                '</span>' +
                `<span class="mq-bracket-middle mq-non-leaf" style="margin-left:${leftSymbol.width};margin-right:${rightSymbol.width}">&0</span>` +
                `<span class="mq-bracket-r mq-scaled mq-paren${this.side === 'left' ? ' mq-ghost' : ''}" style="width:${rightSymbol.width}">` +
                rightSymbol.html +
                '</span>' +
                '</span>';
        return super.html();
    ...
    mathspeak(opts) {
        const open = this.sides.left.ch, close = this.sides.right.ch;
        if (open === '|' && close === '|') {
...
export class MathFunction extends BracketMixin(MathCommand) {
    ...
    setAriaLabel() {
        ...
        if (this.ends.right)
            this.ends.right.ariaLabel = `${this.ariaLabel ?? ''} parameter`;
    }
    ...
    updateFirst() {
        if (this.left &&
            ...
            this.elements.first.classList.add('mq-first');
        else
            this.elements.first.classList.remove('mq-first');
    }
    addToElements(el) {
        this.elements.add(el);
        const children = new VNode(this.elements.children().last).children();
        this.delims = [children.contents[0], children.contents[2]];
    ...
    html() {
        this.htmlTemplate =
            '<span class="mq-non-leaf mq-math-function">' +
                ...
                SVG_SYMBOLS['('].html +
                '</span>' +
                `<span class="mq-bracket-middle mq-non-leaf" style="margin-left:${SVG_SYMBOLS['('].width};margin-right:${SVG_SYMBOLS[')'].width}">&1</span>` +
                `<span class="mq-bracket-r mq-scaled mq-paren${this.side === 'left' ? ' mq-ghost' : ''}" style="width:${SVG_SYMBOLS[')'].width}">` +
                SVG_SYMBOLS[')'].html +
                '</span>' +
                '</span>' +
                '</span>';
        ...
        this.blocks[0].writeHandler = (cursor, ch) => {
            if (ch === '_' || ch === '^')
                return false;
            ...
            if (!cursor.left &&
                LatexCmds[`${this.ctrlSeq.slice(1)}${ch}`]?.prototype instanceof
                    MathFunction) {
    ...
    enterContentBlock(dir, cursor) {
        if (dir === 'left')
            cursor.insAtLeftEnd(this.blocks[1]);
        else
    ...
    deleteSide(side, outward, cursor) {
        if (side === 'left' && !(this.blocks[0].isEmpty() && this.blocks[1].isEmpty())) {
            ...
            if (!this.blocks[0].isEmpty() && this.blocks[0].ends.left instanceof SupSub) {
                ...
                for (const supOrSub of ['sub', 'sup']) {
                    const src = origSupSub[supOrSub];
                    if (src) {
                        if (supsub)
                            cursor.insRightOf(supsub);
                        const newSupOrSub = new LatexCmds[supOrSub === 'sub' ? 'subscript' : 'superscript']();
            ...
            this.remove();
            if (supsub)
                cursor.insLeftOf(supsub);
            else if (brack)
                cursor.insLeftOf(brack);
        }
        else {
            if (side === 'left')
                cursor.controller.aria.queue(this);
            else
                cursor.controller.aria.queue('right parenthesis');
    ...
    finalizeTree() {
        ...
        super.finalizeTree();
        if (!inserted) {
            this.blocks[0].deleteOutOf = (dir, cursor) => {
                ...
                if (!cursor.left &&
                    LatexCmds[this.ctrlSeq.slice(1, -1)]?.prototype instanceof
                        MathFunction) {
            ...
            this.blocks[1].deleteOutOf = (dir, cursor) => {
    ...
    text() {
        return `${this.left instanceof Letter ? ' ' : ''}${this.ctrlSeq.slice(1)}${this.blocks[0]?.text() ?? ''}(${this.blocks[1]?.text() ?? ''})`;
    }
    mathspeak() {
        return `${this.ariaLabel ?? ''} ${this.blocks[0].mathspeak()} left parenthesis, ${this.blocks[1].mathspeak()}, right parenthesis`;
    }
    parser() {
        ...
        return Parser.optWhitespace
            ...
            .or(latexMathParser.block.map((block) => {
            block.children().adopt(this.blocks[1], this.blocks[1].ends.right);
        }))))
            .or(Parser.succeed(this))))
            .result(this);
...
export const latexMathParser = (() => {
    ...
    const variable = Parser.letter.map((c) => new Letter(c));
    const digit = Parser.regex(/^\d/).map((c) => new Digit(c));
    const symbol = Parser.regex(/^[^${}\\_^]/).map((c) => new VanillaSymbol(c));
    const controlSequence = Parser.regex(/^[^\\a-eg-zA-Z]/) // hotfix #164; match MathBlock::write
        .or(Parser.string('\\').then(Parser.regex(/^[a-z]+/i)
