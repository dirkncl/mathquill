...
import { mqCmdId, LatexCmds, CharCmds, otherDir } from '../constants.js';
...
import { deleteSelectTowardsMixin } from '../mixins.js';
import { BlockFocusBlur } from '../services/focusBlur.js';
...
export class TextBlock extends BlockFocusBlur(deleteSelectTowardsMixin(TNode)) {
    ...
    addToElements(el) {
        ...
        this.ends.left?.addToElements(this.elements.first.firstChild);
    }
    createLeftOf(cursor) {
    ...
    unselectInto(dir, cursor) {
        ...
        const newTextPc = cursor[dir].splitRight(this.anticursorPosition);
        if (dir === 'left')
    ...
    seek(pageX, cursor) {
        ...
        const avgChWidth = (this.elements.firstElement.getBoundingClientRect().width -
            parseFloat(cursorStyle.paddingLeft) -
            parseFloat(cursorStyle.paddingRight)) /
            this.text().length;
        const approxPosition = Math.round((pageX - this.elements.firstElement.getBoundingClientRect().left) / avgChWidth);
        if (approxPosition <= 0)
        ...
        if (numericDir * displ < -numericDir * prevDispl)
            cursor[otherDir(dir)]?.moveTowards(otherDir(dir), cursor);
        if (!cursor.anticursor) {
            ...
            if (this.anticursorPosition === cursorPosition) {
                ...
                let newTextPc;
                if (this.anticursorPosition < cursorPosition) {
                    newTextPc = cursor.left.splitRight(this.anticursorPosition);
                    cursor.left = newTextPc;
                }
                else {
                    newTextPc = cursor.right.splitRight(this.anticursorPosition - cursorPosition);
                }
                cursor.anticursor = new Point(this, newTextPc.left, newTextPc);
                cursor.anticursor.ancestors = {};
                for (let ancestor = cursor.anticursor; ancestor.parent; ancestor = ancestor.parent) {
    ...
    fuseChildren() {
        ...
        const textPcDom = this.elements.first.firstChild;
        if (!textPcDom)
            return;
        // nodeType === 3 is a text node.
        // See https://developer.mozilla.org/en-US/docs/Web/API/Node/nodeType.
        if (textPcDom.nodeType !== 3)
            throw new Error('only node in TextBlock span must be a Text node');
...
export class TextPiece extends TNode {
    ...
    splitRight(i) {
        ...
        if (this.parent)
            newPc.adopt(this.parent, this, this.right);
        if (this.dom)
    ...
    moveTowards(dir, cursor) {
        ...
        const from = this[otherDir(dir)];
        if (from)
            from.insTextAtDirEnd(ch, dir);
        else
            new TextPiece(ch).createDir(otherDir(dir), cursor);
    ...
    selectTowards(dir, cursor) {
        ...
        if (anticursor?.[dir] === this) {
            ...
            if (from)
                ...
                if (cursor.selection)
                    newPc.elements.insDirOf(otherDir(dir), cursor.selection.elements);
            }
        ...
        LatexCmds.textrm =
            LatexCmds.textup =
                CharCmds['"'] =
                    LatexCmds.textmd =
                        TextBlock;
const makeTextBlock = (latex, tagName, attrs, ariaLabel) => class extends TextBlock {
    htmlTemplate;
    constructor() {
        super();
        this.ctrlSeq = latex;
...
LatexCmds.sf = LatexCmds.textsf = makeTextBlock('\\textsf', 'span', 'class="mq-sans-serif mq-text-mode"', 'Sans serif font');
LatexCmds.tt = LatexCmds.texttt = makeTextBlock('\\texttt', 'span', 'class="mq-monospace mq-text-mode"', 'Mono space font');
LatexCmds.textsc = makeTextBlock('\\textsc', 'span', 'style="font-variant:small-caps" class="mq-text-mode"', 'Variable font');
LatexCmds.uppercase = makeTextBlock('\\uppercase', 'span', 'style="text-transform:uppercase" class="mq-text-mode"', 'Uppercase');
LatexCmds.lowercase = makeTextBlock('\\lowercase', 'span', 'style="text-transform:lowercase" class="mq-text-mode"', 'Lowercase');
