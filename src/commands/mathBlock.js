import { LatexCmds, CharCmds, otherDir } from '../constants.js';
import { RootBlockMixin } from '../mixins.js';
import { Parser } from '../services/parser.util.js';
import { BlockFocusBlur } from '../services/focusBlur.js';
import { VanillaSymbol, MathElement, MathCommand, Letter, Digit, latexMathParser } from './mathElements.js';
...
export const writeMethodMixin = (Base) => class extends Base {
    ...
    write(cursor, ch) {
        if (this.writeHandler?.(cursor, ch))
            return;
...
export class MathBlock extends BlockFocusBlur(writeMethodMixin(MathElement)) {
    ...
    mathspeak() {
        ...
        return (this.foldChildren([], (speechArray, cmd) => {
            ...
            // six."
            .replace(/(\.)([0-9]+)/g, (_match, p1, p2) => p1 + p2.split('').join(' ').trim()));
    }
    keystroke(key, e, ctrlr) {
        if (ctrlr.options.enableSpaceNavigation &&
    ...
    seek(pageX, cursor) {
        ...
        while (pageX < (node?.elements.firstElement.getBoundingClientRect().left ?? 0))
            node = node?.left;
    ...
    writeLatex(cursor, latex) {
        ...
        if (block && !block.isEmpty() && block.prepareInsertionAt(cursor)) {
            if (cursor.parent)
                block.children().adopt(cursor.parent, cursor.left, cursor.right);
...
export class RootMathCommand extends writeMethodMixin(MathCommand) {
    ...
    createBlocks() {
        ...
        leftEnd.write = (cursor, ch) => {
            ...
            else if (!cursor.right && leftEnd.parent)
                cursor.insRightOf(leftEnd.parent);
            else if (!cursor.left && leftEnd.parent)
                cursor.insLeftOf(leftEnd.parent);
            else
                this.write(cursor, ch);
        };
    }
