...
import { LatexCmds, CharCmds } from '../../constants.js';
import { VNode } from '../../tree/vNode.js';
import { VanillaSymbol, MathCommand } from '../mathElements.js';
import { TextBlock } from '../textElements.js';
    ...
    createBlocks() {
        ...
        leftEnd.write = (cursor, ch) => {
            ...
            if (/[a-z]/i.exec(ch)) {
                ...
                const cmd = leftEnd.parent.renderCommand(cursor);
                cursor.controller.aria.queue(cmd.mathspeak({ createdLeftOf: cursor }));
                if (ch !== '\\' || !leftEnd.isEmpty())
                    cursor.parent?.write(cursor, ch);
                else
                    cursor.controller.aria.alert();
            }
        };
        leftEnd.keystroke = (key, e, ctrlr) => {
            if (key === 'Tab' || key === 'Shift-Tab' || key === 'Enter' || key === 'Spacebar') {
                const cmd = leftEnd.parent.renderCommand(ctrlr.cursor);
