...
import { noop, bindMixin, LatexCmds, CharCmds, BuiltInOpNames } from '../../constants.js';
import { Parser } from '../../services/parser.util.js';
import { Symbol, VanillaSymbol, BinaryOperator, FactorialOrNEQ, Equality, Inequality, MathCommand, Variable, Letter, latexMathParser } from '../mathElements.js';
...
class PlusMinus extends BinaryOperator {
    constructor(ctrlSeq, html, text, mathspeak) {
        ...
        this.siblingCreated = this.siblingDeleted = (opts, dir) => this.contactWeld(opts, dir);
    }
    contactWeld(_opts, dir) {
        const isUnary = (node) => {
            if (node.left) {
                ...
                if (node.left instanceof BinaryOperator || /^[,;:([]$/.test(node.left.ctrlSeq)) {
                    return true;
                }
            }
