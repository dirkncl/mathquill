...
import { bindMixin, LatexCmds, CharCmds, OPP_BRACKS, SVG_SYMBOLS, EMBEDS } from '../../constants.js';
import { Controller } from '../../controller.js';
import { Parser } from '../../services/parser.util.js';
import { RootBlockMixin, DelimsMixin } from '../../mixins.js';
import { Fragment } from '../../tree/fragment.js';
import { InnerMathField } from '../math.js';
import { BinaryOperator, Equality, MathCommand, Symbol, Letter, insLeftOfMeUnlessAtEnd, Fraction, SupSub, UpperLowerLimitCommand, Bracket, latexMathParser, supSubText, MathFunction, intRgx, getCtrlSeqsFromBlock } from '../mathElements.js';

class Style extends MathCommand {
    shouldNotSpeakDelimiters;
    constructor(ctrlSeq, tagName, attrs, ariaLabel, shouldNotSpeakDelimiters) {
    ...
    mathspeak(opts) {
        if (!this.shouldNotSpeakDelimiters || opts?.ignoreShorthand)
...
LatexCmds.overline = LatexCmds.bar = bindMixin(Style, '\\overline', 'span', 'class="mq-non-leaf mq-overline"', 'Overline');
LatexCmds.overrightarrow = bindMixin(Style, '\\overrightarrow', 'span', 'class="mq-non-leaf mq-overarrow mq-arrow-right"', 'Over Right Arrow');
LatexCmds.overleftarrow = bindMixin(Style, '\\overleftarrow', 'span', 'class="mq-non-leaf mq-overarrow mq-arrow-left"', 'Over Left Arrow');
LatexCmds.overleftrightarrow = bindMixin(Style, '\\overleftrightarrow', 'span', 'class="mq-non-leaf mq-overarrow mq-arrow-both"', 'Over Left and Right Arrow');
                        ...
                        if (!opts?.ignoreShorthand && intRgx.test(innerText)) {
                            ...
                            if (/^[+-]?\d{1,3}$/.test(innerText)) {
                                if (/(11|12|13|4|5|6|7|8|9|0)$/.test(innerText))
                                    suffix = 'th';
                                else if (innerText.endsWith('1'))
                                    suffix = 'st';
                                else if (innerText.endsWith('2'))
                                    suffix = 'nd';
                                else if (innerText.endsWith('3'))
                                    suffix = 'rd';
                            }
...
class SummationNotation extends UpperLowerLimitCommand {
    constructor(ch, html, ariaLabel) {
        super(ch, '<span class="mq-large-operator mq-non-leaf">' +
            '<span class="mq-to"><span>&1</span></span>' +
            `<big>${html}</big>` +
            '<span class="mq-from"><span>&0</span></span>' +
            '</span>', [ch.length > 1 ? ch.slice(1) : ch]);
    ...
    LatexCmds.int =
        LatexCmds.integral =
            class extends UpperLowerLimitCommand {
                constructor() {
                    super('\\int ', '<span class="mq-int mq-non-leaf">' +
                        '<big>&int;</big>' +
                        '<span class="mq-supsub mq-non-leaf">' +
...
LatexCmds.hat = class extends MathCommand {
    constructor() {
        super('\\hat', '<span class="mq-non-leaf">' +
...
class NthRoot extends SquareRoot {
    ...
    text() {
        ...
        if (index === '' || index === '2')
            return `sqrt(${this.ends.right?.text() ?? ''})`;
    ...
    mathspeak() {
        ...
        if (this.ends.left)
            this.ends.left.ariaLabel = 'Index';
        if (this.ends.right)
            this.ends.right.ariaLabel = 'Radicand';
        if (indexMathspeak === '3')
...
class DiacriticAbove extends MathCommand {
    constructor(ctrlSeq, symbol, textTemplate) {
        super(ctrlSeq, '<span class="mq-non-leaf">' +
...
class Binomial extends DelimsMixin(MathCommand) {
    constructor() {
        super('\\binom', '<span class="mq-bracket-container mq-non-leaf">' +
            `<span class="mq-paren mq-bracket-l mq-scaled" style="width:${SVG_SYMBOLS['('].width}">` +
            SVG_SYMBOLS['('].html +
            '</span>' +
            `<span class="mq-bracket-middle mq-non-leaf" style="margin-left:${SVG_SYMBOLS['('].width};margin-right:${SVG_SYMBOLS[')'].width}">` +
...
LatexCmds.editable = LatexCmds.MathQuillMathField = class extends MathCommand {
    ...
    field;
    constructor() {
        super('\\MathQuillMathField', '<span class="mq-editable-field">' + '<span class="mq-root-block" aria-hidden="true">&0</span>' + '</span>');
    }
    parser() {
    ...
    finalizeTree(options) {
        if (!this.ends.left)
            throw new Error('Missing left end finalizing editable tree');
        ...
        const pushDownAriaHidden = (node) => {
            ...
            if (node.parentNode && !node.classList.contains('mq-root-block'))
    ...
    registerInnerField(innerFields) {
        if (!this.field)
            throw new Error('Unable to register editable without field');
