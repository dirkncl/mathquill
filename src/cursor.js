...
export class Cursor extends Point {
    ...
    blink = () => this.element.classList.toggle('mq-blink');
        ...
        if (this.intervalId) {
            ...
            if (this.right) {
                if (this.selection && this.selection.ends.left?.left === this.left)
                    ...
                    this.right.elements.first.before(this.element);
            }
            else
                this.parent?.elements.firstElement.append(this.element);
            this.parent?.focus();
        }
    ...
    insDirOf(dir, el) {
        ...
        if (dir === 'left')
            el.elements.first.before(this.element);
        else
            el.elements.last.after(this.element);
        if (el.parent)
    ...
    insAtDirEnd(dir, el) {
        if (dir !== 'left' && dir !== 'right')
            throw new Error('a direction was not passed');
        if (dir === 'left')
            el.elements.firstElement.prepend(this.element);
        else
            el.elements.lastElement.append(this.element);
    ...
    unwrapGramp() {
        ...
        if (!this.right) {
            ...
            if (this.left)
                ...
                while (!this.right) {
                    ...
                    if (this.parent)
                        this.right = this.parent.ends.left;
                    else {
        ...
        if (this.right)
            this.insLeftOf(this.right);
        else if (greatgramp)
            this.insAtRightEnd(greatgramp);
        gramp?.elements.remove();
        if (gramp?.left?.siblingDeleted)
            gramp.left.siblingDeleted(this.options, 'right');
        if (gramp?.right?.siblingDeleted)
            gramp.right.siblingDeleted(this.options, 'left');
    }
    startSelection() {
    ...
    select() {
        if (this.left === this.anticursor?.left && this.parent === this.anticursor?.parent)
            return false;
        if (!this.anticursor?.ancestors)
            throw new Error('selection not well formed');
        // Find the lowest common ancestor (`lca`), and the ancestor of the cursor
        // whose parent is the LCA (which will be an end of the selection fragment).
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        let ancestor = this, lca = null;
        ...
        if (!lca)
            throw new Error('cursor and anticursor must be in the same tree');
        ...
        // of the selection.
        let leftEnd, rightEnd, dir = 'right';
        ...
        if (ancestor.left !== antiAncestor) {
            for (let rightward = ancestor; rightward; rightward = rightward.right) {
        ...
        if (selectionEndDir)
            this.insDirOf(dir, selectionEndDir);
