...
import { mqCmdId, mqBlockId, noop } from '../constants.js';
import { TNode } from '../tree/node.js';
import { Letter, Digit } from '../commands/mathElements.js';
import { TextBlock } from '../commands/textElements.js';
...
export const MouseEventController = (Base) => class extends Base {
    ...
    delegateMouseEvents() {
        ...
        this.mouseDownHandler = (e) => {
            const rootEl = e.target?.closest('.mq-root-block');
            const root = TNode.byId.get(parseInt((rootEl?.getAttribute(mqBlockId) || ultimateRootEl.getAttribute(mqBlockId)) ?? '0'));
            if (!root?.controller) {
            ...
            if (cursor.options.ignoreNextMousedown(e))
                ...
                cursor.options.ignoreNextMousedown = () => false;
            ...
            if (e.detail === 3) {
                ...
                ctrlr.seek(e.target, e.pageX);
                if (!cursor.right && cursor.left?.parent === root)
                    ctrlr.moveLeft();
                ctrlr.withIncrementalSelection((selectDir) => {
                    if (cursor.right instanceof Letter) {
                        ...
                        while (cursor.left && cursor.left instanceof Digit)
                            ctrlr.moveLeft();
                        cursor.startSelection();
                        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
                        while (cursor.right && cursor.right instanceof Digit)
                            selectDir('right');
                    }
                    else {
                        cursor.startSelection();
            ...
            if (ctrlr.blurred)
                textarea?.focus();
            cursor.blink = noop;
            ctrlr.seek(e.target, e.pageX).cursor.startSelection();
    ...
    seek(target, pageX) {
        ...
        if (target) {
            ...
            if (!nodeId) {
                ...
                nodeId = parseInt((targetParent?.getAttribute(mqBlockId) || targetParent?.getAttribute(mqCmdId)) ?? '0');
            }
        }
        const node = nodeId ? TNode.byId.get(nodeId) : this.root;
        if (!node)
            throw new Error('nodeId is not the id of a TNode that exists');
