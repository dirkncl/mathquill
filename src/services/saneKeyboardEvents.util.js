...
export const saneKeyboardEvents = (() => {
    ...
    const isLowercaseAlphaCharacter = (s) => s.length === 1 && s >= 'a' && s <= 'z';
    ...
    return (textarea, controller) => {
        ...
        const select = (text) => {
            textarea.value = text;
            if (text && textarea instanceof HTMLTextAreaElement)
                textarea.select();
        };
        const handleKey = (key, e) => {
            if (controller.options.overrideKeystroke)
                controller.options.overrideKeystroke(key, e);
            else
                controller.keystroke(key, e);
        };
        // Event handlers
        const onKeydown = (e) => {
        ...
        const onPaste = (e) => {
            ...
            textarea.value = '';
            if (text) {
                if (controller.options.overridePaste)
                    controller.options.overridePaste(text);
                else
        ...
        textarea.addEventListener('cut', () => {
            if (controller.options.overrideCut)
                controller.options.overrideCut();
            else
                controller.cut();
        });
        textarea.addEventListener('copy', () => {
            if (controller.options.overrideCopy)
                controller.options.overrideCopy();
            else
                controller.copy();
        });
