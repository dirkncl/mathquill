...
export const TextAreaController = (Base) => class extends Base {
    ...
    createTextarea() {
        ...
        const tabbable = typeof this.options.tabbable === 'boolean' ? this.options.tabbable : this.KIND_OF_MQ !== 'StaticMath';
        ...
        if (!this.options.tabbable && this.KIND_OF_MQ === 'StaticMath')
            ...
            textarea.setAttribute('aria-hidden', 'undefined');
    ...
    editablesTextareaEvents() {
        if (this.textarea) {
            const { select } = saneKeyboardEvents(this.textarea, this);
