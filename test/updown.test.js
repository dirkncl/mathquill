...
suite('up/down', function () {
    ...
    test('up/down in out of subscript', function () {
        ...
        const sub = rootBlock.ends.right, subBlock = sub?.ends.left;
    ...
    test('up/down into and within fraction', function () {
        ...
        const frac = rootBlock.ends.left, numer = frac?.ends.left, denom = frac?.ends.right;
    ...
    test('nested subscripts and fractions', function () {
        ...
        const exp = rootBlock.ends.right, expBlock = exp?.ends.left, half = expBlock?.ends.left, halfDenom = half?.ends.right;
        mq.keystroke('Left');
        ...
        const derivative = rootBlock.ends.left, dxBlock = derivative?.ends.right, sub = dxBlock?.ends.right, subBlock = sub?.ends.left, subFrac = subBlock?.ends.left, subFracNumer = subFrac?.ends.left, subFracDenom = subFrac?.ends.right;
        ...
        assert.equal(cursor.right, sub, "cursor up up from subscript fraction denominator that's not at right end goes before subscript");
        if (!subBlock)
            throw new Error('sub block is undefined');
        ...
        assert.equal(cursor.left, sub, 'cursor up up from subscript fraction denominator that is at right end goes after subscript');
    });
    test('\\MathQuillMathField{} in a fraction', function () {
        ...
        if (!(innerEditableSpan instanceof HTMLElement))
            throw new Error('inner editable field does not exist');
        const inner = MQ(innerEditableSpan);
