...
suite('focusBlur', function () {
    ...
    const assertHasFocus = (mq, name, invert) => {
        assert.ok(invert !== (mq.el().querySelector('textarea') === document.activeElement), name + (invert ? ' does not have focus' : ' has focus'));
    };
    suite('handlers can shift focus away', function () {
        ...
        const triggerUpOutOf = (mq) => {
            mq
                ...
                ?.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowUp', which: 38, keyCode: 38, bubbles: true }));
    ...
    test('select behaves correctly after blurring and re-focusing', function (done) {
        ...
        setTimeout(() => {
            ...
            if (document.hasFocus()) {
                ...
                console.warn('The test "select behaves correctly after blurring and re-focusing" needs the ' +
