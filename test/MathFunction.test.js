...
import { VNode } from '../src/tree/vNode.js';
import { MathFunction, latexMathParser } from '../src/commands/mathElements.js';
...
suite('MathFunction', function () {
    ...
    suite('parsing', function () {
        test('general', function () {
            ...
            assertParsesLatex('\\sinh_{3^2_4}^{y^2+z}\\left(\\pi x\\right)', '\\sinh_{3_4^2}^{y^2+z}\\left(\\pi x\\right)');
    ...
    suite('latex rendering', function () {
        ...
        test('basic latex output', function () {
            const tree = latexMathParser
                .parse('\\sin_2^3\\left(x^2+3\\right)')
                .postOrder('finalizeTree', mq.options);
    ...
    suite('behavior as a left bracket', function () {
        suite('as initial left bracket', function () {
            ...
            setup(function () {
                ...
                if (!(fcn instanceof MathFunction))
                    throw new Error('cursor parent parent is not MathFunction');
            ...
            test('typing outside ghost paren solidifies ghost', function () {
                ...
                assert.ok(!endBracket.classList.contains('mq-ghost'), 'right parenthesis is solid after typing to right');
