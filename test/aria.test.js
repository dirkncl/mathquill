import MathQuill from '../src/publicapi.js';
...
suite('aria', function () {
    ...
    test('Tabbable static math aria-hidden', function () {
        ...
        assert.equal(textarea[0].getAttribute('aria-labelledby'), mathspeak[0].id, 'textarea is aria-labelledby mathspeak region');
    });
    test('MathQuillMathField aria-hidden', function () {
        ...
        assert.equal(document.querySelectorAll('textarea[tabindex="-1"]').length, 1, 'The static math textarea is not tabbable.');
        ...
        for (const elt of container.querySelectorAll('*')) {
            ...
            assert.ok(false, 'All children with text content are aria-hidden, have an aria-hidden parent, or are part of mathspeak');
        }
    ...
    test('testing aria-label for interactive math', function (done) {
        if (document.hasFocus()) {
            ...
            mq.blur();
            setTimeout(() => {
                assert.equal(mq.__controller.mathspeakSpan?.textContent, 'Math Input: StartSquareRoot, x , EndSquareRoot');
                done();
            });
        }
        else {
            console.warn('The test "testing aria-label for interactive math" needs the document to have focus.\n' +
    ...
    test('testing aria-label for static math', function () {
        ...
        assert.equal(staticMath.__controller.mathspeakSpan?.textContent, 'y equals StartFraction, 2 x Over 3 y , EndFraction');
        assert.equal('', staticMath.getAriaLabel());
        staticMath.setAriaLabel('Static Label');
        assert.equal(staticMath.__controller.mathspeakSpan?.textContent, 'Static Label: y equals StartFraction, 2 x Over 3 y , EndFraction');
