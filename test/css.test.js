...
suite('CSS', function () {
    ...
    test('test florin spacing', function () {
        ...
        assert.ok(parseFloat(computedStyle?.marginRight ?? '0') > parseFloat(computedStyle?.marginLeft ?? '0'), 'florin right margin should be greater than left margin');
    });
    test('unary PlusMinus before separator', function () {
        ...
        const assertBinaryOperator = (i, s) => {
            assert.ok(isBinaryOperator(i), '"' + s + '" should be binary');
        };
        const assertUnaryOperator = (i, s) => {
    ...
    test('operator name spacing, e.g., ker x', function () {
        ...
        assert.ok(!supsub?.classList.contains('mq-after-operator-name'));
        const mqEl = mq.el();
        while (mqEl.firstChild)
            mqEl.firstChild.remove();
    });
});
