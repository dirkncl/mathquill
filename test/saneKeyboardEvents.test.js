import { noop } from '../src/constants.js';
import { saneKeyboardEvents } from '../src/services/saneKeyboardEvents.util.js';
import { Options } from '../src/options.js';
import { MathField } from '../src/commands/math.js';
...
suite('saneKeyboardEvents', function () {
    ...
    test('normal keys', function (done) {
        ...
        el.dispatchEvent(new KeyboardEvent('keydown', { key: 'a', shiftKey: true, which: 65, keyCode: 65, bubbles: true }));
        el.dispatchEvent(new InputEvent('input', { data: 'a', bubbles: true }));
        el.dispatchEvent(new KeyboardEvent('keypress', { key: 'a', shiftKey: true, which: 65, keyCode: 65, bubbles: true }));
        el.value = 'a';
    });
    test('normal keys without keypress', function (done) {
        ...
        el.dispatchEvent(new KeyboardEvent('keydown', { key: 'a', shiftKey: true, which: 65, keyCode: 65, bubbles: true }));
        el.dispatchEvent(new InputEvent('input', { data: 'a', bubbles: true }));
        el.dispatchEvent(new KeyboardEvent('keyup', { key: 'a', shiftKey: true, which: 65, keyCode: 65, bubbles: true }));
        el.value = 'a';
    });
    ...
    suite('paste', function () {
        ...
        test('paste after keydown/keypress', function (done) {
            ...
            saneKeyboardEvents(el, ctrlr);
            el.dispatchEvent(new KeyboardEvent('keydown', { key: 'v', which: 86, keyCode: 86, ctrlKey: true, bubbles: true }));
            el.dispatchEvent(new KeyboardEvent('keypress', { key: 'v', which: 118, keyCode: 118, ctrlKey: true, bubbles: true }));
            const event = new ClipboardEvent('paste', { clipboardData: new DataTransfer(), bubbles: true });
