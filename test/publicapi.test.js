...
suite('Public API', function () {
    ...
    suite('*OutOf handlers', function () {
        const testHandlers = (title, mathFieldMaker) => {
            test(title, function () {
                let enterCounter = 0, upCounter = 0, moveCounter = 0, deleteCounter = 0, dir = null;
        ...
        testHandlers('.config() directly on a \\MathQuillMathField{} in a MQ.StaticMath using .innerFields', (options) => {
            const el = document.createElement('span');
            el.textContent = '\\MathQuillMathField{}';
            document.getElementById('mock')?.append(el);
        ...
        test('overflow triggers automatic horizontal scroll', function (done) {
            ...
            setTimeout(() => {
                ...
                try {
                    ...
                    assert.ok(mqEl.getBoundingClientRect().right > cursor.element.getBoundingClientRect().right, 'cursor right end is inside the field');
                }
                catch (error) {
                    done(error);
                    return;
    ...
    suite('enableSpaceNavigation', function () {
        ...
        test('space behaves like tab with default opts', function () {
            ...
            assert.equal(cursor.right, undefined, 'right of the cursor is ' + (cursor.right?.toString() ?? 'undefined'));
        ...
        test('space behaves like tab when enableSpaceNavigation is true', function () {
            ...
            mq.keystroke('Left');
            mq.keystroke('Spacebar');
            assert.equal(cursor.left?.parent, rootBlock, 'parent of the cursor is  ' + (cursor.left?.ctrlSeq ?? 'undefined'));
            ...
            assert.equal(cursor.right, rootBlock.ends.left, 'parent of rootBlock is ' + (cursor.right?.toString() ?? 'undefined'));
        });
    ...
    suite('keyboard event overrides', function () {
        test('can intercept key events', function () {
            ...
            mq
                ...
                ?.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowLeft', which: 37, keyCode: 37, bubbles: true }));
