...
import { Bracket } from '../src/commands/mathElements.js';
import { prayWellFormed } from '../src/constants.js';
...
suite('typing with auto-replaces', function () {
    ...
    suite('auto-expanding parens', function () {
        ...
        function backspacingTests() {
            ...
            function assertParenBlockNonEmpty() {
                ...
                assert.ok(!parenBlock[0].classList.contains('mq-empty'), 'paren block auto-expanded, should no longer be gray');
            }
            ...
            suite('pipes (backspacing)', function () {
                ...
                test('rendering mismatched paren/pipe group 1+|2+3)+4 from LaTeX ' +
                    'then backspacing close-paren then open-pipe', function () {
                    mq.latex('1+\\left|2+3\\right)+4');
                    assertLatex('1+\\left|2+3\\right)+4');
                    mq.keystroke('Left Left Backspace');
                    assertLatex('1+\\left|2+3+4\\right|');
                ...
                test('rendering mismatched paren/pipe group 1+(2+3|+4 from LaTeX ' +
                    'then backspacing close-pipe then open-paren', function () {
                    mq.latex('1+\\left(2+3\\right|+4');
                    assertLatex('1+\\left(2+3\\right|+4');
                    mq.keystroke('Left Left Backspace');
                    assertLatex('1+\\left(2+3+4\\right)');
        ...
        suite('typing outside ghost paren', function () {
            test('typing outside ghost paren solidifies ghost 1+(2+3)', function () {
                ...
                assert.ok(bracket instanceof Bracket &&
                    bracket.elements.children().last.classList.contains('mq-ghost'));
                ...
                assertLatex('\\left(1+2+3\\right)+4');
                assert.ok(bracket instanceof Bracket &&
                    bracket.elements.children().first.classList.contains('mq-ghost'));
                assert.ok(bracket instanceof Bracket &&
                    !bracket.elements.children().last.classList.contains('mq-ghost'));
            });
    ...
    suite('inequalities', function () {
        ...
        const assertFullyFunctioningInequality = (nonStrict, strict) => {
